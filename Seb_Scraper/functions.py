...
def get_data_typeA(i, link):
    ...
    if a == True:
        ...
        Odd_1 = fffi('//*[@id="odds-data-table"]/div[1]/table/tfoot/*[@class="aver"]/td[2]')
        Odd_2 = fffi('//*[@id="odds-data-table"]/div[1]/table/tfoot/*[@class="aver"]/td[3]')
        match = ffi('//*[@id="col-content"]/h1')  # match teams
        final_score = ffi('//*[@id="event-status"]')
        print(match, Odd_1, Odd_2, final_score)
        L = L + [(match, Odd_1, Odd_2, final_score)]
        return (L)
...
def scrape_current_tournament_typeA(sport, tournament, country, SEASON, max_page=25):
    ...
    data_df.to_csv('./{}_FULL/{}_{}_FULL.csv'.format(tournament, tournament, SEASON), sep=';', encoding='utf-8',
                   index=False)
    data_df[
...
def scrape_oddsportal_historical(sport='football', country='france', league='ligue-1', start_season='2019-2020',
    ...
    if sport in ['baseball', 'esports', 'basketball', 'darts', 'american-football', 'volleyball']:
        df = scrape_league_typeA(Season=start_season, sport=sport, country1=country, tournament1=league,
                                 nseason=nseasons, current_season='yes', max_page=max_page)
        df = create_clean_table_two_ways(df)
    # elif sport in ['tennis']:
...
def scrape_oddsportal_specific_season(sport='football', country='france', league='ligue-1', season='2019-2020',
    ...
    if sport in ['baseball', 'esports', 'basketball', 'darts', 'american-football', 'volleyball']:
        df = scrape_current_tournament_typeA(sport, tournament=league, country=country, SEASON=season, max_page=max_page)
        df = create_clean_table_two_ways(df)
    # elif sport in ['tennis']:
...
def scrape_nfl(sport='american-football', country="usa",  league='nfl', season='2022-2023', max_page=8):
    ...
    df.to_csv('./{}/{}_{}.csv'.format(sport, league, season), sep=',', encoding='utf-8', index=False)